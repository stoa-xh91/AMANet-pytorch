...
from ..poolers import ROIPooler, MultiROIPooler
from ..proposal_generator.proposal_utils import add_ground_truth_to_proposals
from ..sampling import subsample_labels
from .box_head import build_box_head
from .fast_rcnn import FastRCNNOutputLayers, FastRCNNOutputs
from .keypoint_head import build_keypoint_head, keypoint_rcnn_inference, keypoint_rcnn_loss, keypoint_rcnn_inter_part_loss,keypoint_rcnn_hybrid_loss
...
class StandardROIHeads(ROIHeads):
    ...
    def _init_mask_head(self, cfg):
        # fmt: off
        self.mask_on           = cfg.MODEL.MASK_ON and cfg.MODEL.ROI_MASK_HEAD.RCNN_HEAD_ON
        if not self.mask_on:
            return
    ...
    def _init_keypoint_head(self, cfg):
        # fmt: off
        self.keypoint_on                         = cfg.MODEL.KEYPOINT_ON and cfg.MODEL.ROI_KEYPOINT_HEAD.RCNN_HEAD_ON
        ...
        self.RELATION_ON                = cfg.MODEL.ROI_KEYPOINT_HEAD.RELATION_ON
        self.multi_rois = cfg.MODEL.ROI_KEYPOINT_HEAD.MULTI_ROIS
        # fmt: on

        in_channels = [self.feature_channels[f] for f in self.in_features][0]
        if self.multi_rois:
            self.keypoint_pooler = MultiROIPooler(
                output_size=[[14,14],[14,14],[14,14],[14,14]],
                scales=pooler_scales,
                sampling_ratio=sampling_ratio,
                pooler_type=pooler_type,
            )
    ...
    def _forward_keypoint(self, features, instances):
        ...
        if not self.keypoint_on:
            return {} if self.training else instances
        if self.RELATION_ON:
            return self._forward_pr_keypoint(features, instances)
        ...
        if self.training:
            ...
            loss, inter_loss = keypoint_rcnn_hybrid_loss(
                keypoint_logits,
                proposals,
                normalizer=None if self.normalize_loss_by_visible_keypoints else normalizer,
            )
            ...
            return {"loss_keypoint": loss * self.keypoint_loss_weight,
                    "loss_mse_keypoint": inter_loss * self.keypoint_loss_weight}
        else:
            pred_boxes = [x.pred_boxes for x in instances]
            ...
            #     # set keypoint_logits to empty tensors
            #     keypoint_logits = torch.zeros(size=(0, 0, 0, 0), device=keypoint_features.device)
            keypoint_rcnn_inference(keypoint_logits, instances)
            return instances

    def _forward_pr_keypoint(self, features, instances):
        """
        Forward logic of the keypoint prediction branch.
        ...
        if self.training:
            ...
            keypoint_features = self.keypoint_pooler(features, proposal_boxes)
            keypoint_logits, inter_keypoint_logits = self.keypoint_head(keypoint_features)
            ...
            inter_loss = keypoint_rcnn_loss(
                inter_keypoint_logits,
                proposals,
                normalizer=None if self.normalize_loss_by_visible_keypoints else normalizer,
            )
            # loss, inter_loss = keypoint_rcnn_inter_part_loss(
            #     keypoint_logits,
            #     inter_keypoint_logits,
            #     proposals,
            #     normalizer=None if self.normalize_loss_by_visible_keypoints else normalizer,
            ...
            return {"loss_keypoint": loss * self.keypoint_loss_weight, "loss_inter_keypoint": inter_loss* self.keypoint_loss_weight}
        else:
            # pred_boxes = [x.pred_boxes for x in instances]
            # keypoint_features = self.keypoint_pooler(features, pred_boxes)
            # keypoint_logits,_ = self.keypoint_head(keypoint_features)
            # keypoint_rcnn_inference(keypoint_logits, instances)
