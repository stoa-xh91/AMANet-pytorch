...
@ROI_DENSEPOSE_HEAD_REGISTRY.register()
class DensePoseAMAHead_old(nn.Module):
    def __init__(self, cfg, input_channels):
        super(DensePoseAMAHead, self).__init__()
        # fmt: off
        self.dp_semseg_on = cfg.MODEL.ROI_DENSEPOSE_HEAD.SEMSEG_ON
...
@ROI_DENSEPOSE_HEAD_REGISTRY.register()
class DetDistillAMAPredictor(nn.Module):

    NUM_ANN_INDICES = 15

    def __init__(self, cfg, input_channels):
        super(DetDistillAMAPredictor, self).__init__()
        self.dp_keypoints_on = cfg.MODEL.ROI_DENSEPOSE_HEAD.KPT_ON
        dim_in = input_channels
        dim_out_ann_index = self.NUM_ANN_INDICES
        dim_out_patches = cfg.MODEL.ROI_DENSEPOSE_HEAD.NUM_PATCHES + 1
        ...
        part_surface_transformer.append(nn.Linear(index_weight_size, index_weight_size))
        self.part_surface_transformer = nn.Sequential(*part_surface_transformer)
        dim_in = cfg.MODEL.ROI_DENSEPOSE_HEAD.CONV_HEAD_DIM
        for i in range(2):
            layer = Conv2d(dim_in, dim_out_ann_index, 3, stride=1, padding=1)
            layer_name = self._get_layer_name(i, 'ann_index')
    ...
    def generate_surface_weights_from_bbox(self, bbox_weight):
        n_out, n_in = bbox_weight.size(0), bbox_weight.size(1)

        body_surface_weight = torch.matmul(self.bbox_surface_transfer_matrix, bbox_weight)
        body_surface_weight = self.bbox_surface_transformer(body_surface_weight)
        body_surface_weight = body_surface_weight.reshape((self.bbox_surface_transfer_matrix.size(0), self.dim_in, 3, 3))
        return body_surface_weight
    def generate_surface_weights_from_part(self, part_weight):
        n_out, n_in = part_weight.size(0), part_weight.size(1)
        part_weight = part_weight.reshape(n_out,-1)
        part_surface_weight = torch.matmul(self.part_surface_transfer_matrix, part_weight)
        part_surface_weight = self.part_surface_transformer(part_surface_weight)
        part_surface_weight = part_surface_weight.reshape((self.part_surface_transfer_matrix.size(0), self.dim_in, 3, 3))
        return part_surface_weight
    ...
    def forward(self, head_outputs, inter_outputs, task_params=None):
        ...
        for i in range(2):
            ...
            inter_out.append(inter_ann_index)
            if i == 0:
                body_surface_weight = self.generate_surface_weights_from_bbox(task_params)
            else:
                body_surface_weight = self.generate_surface_weights_from_part(self.ann_index_layer.weight)
            ...
            inter_ann_index = getattr(self, layer_name)(inter_features[i])
            inter_out.append(inter_ann_index)
        # index_uv = (index_uv+inter_out1[1]+inter_out2[1])*0.33
        return (ann_index, index_uv, u, v, m), inter_out1, inter_out2

@ROI_DENSEPOSE_HEAD_REGISTRY.register()
class TaskTransformerAMAPredictor(nn.Module):

    NUM_ANN_INDICES = 15

    def __init__(self, cfg, input_channels):
        super(TaskTransformerAMAPredictor, self).__init__()
        self.dp_keypoints_on = cfg.MODEL.ROI_DENSEPOSE_HEAD.KPT_ON
        dim_in = input_channels
        dim_out_ann_index = self.NUM_ANN_INDICES
        dim_out_patches = cfg.MODEL.ROI_DENSEPOSE_HEAD.NUM_PATCHES + 1
        ...
        self.bbox_parameter_encoder = MultiHeadAttention(index_weight_size, 1)
        self.bbox_parameter_decoder = MultiHeadAttention(index_weight_size, 1)
        self.bbox_to_param_predictor = nn.Linear(index_weight_size, index_weight_size)
        #
        self.part_surface_transfer_matrix = Parameter(torch.Tensor(
            dim_out_patches, dim_out_ann_index))
        part_weight_size = dim_in*3*3
        ...
        self.part_parameter_encoder = MultiHeadAttention(index_weight_size, 1)
        self.part_parameter_decoder = MultiHeadAttention(index_weight_size, 1)

        self.part_to_param_predictor = nn.Linear(index_weight_size, index_weight_size)
        dim_in = cfg.MODEL.ROI_DENSEPOSE_HEAD.CONV_HEAD_DIM
        for i in range(2):
            layer = Conv2d(dim_in, dim_out_ann_index, 3, stride=1, padding=1)
            layer_name = self._get_layer_name(i, 'ann_index')
    ...
    def generate_surface_weights_from_bbox(self, bbox_weight):
        ...
        n_out, n_in = bbox_weight.size(0), bbox_weight.size(1)
        bbox_weight = torch.matmul(self.bbox_surface_transfer_matrix, bbox_weight)
        bbox_weight = self.bbox_surface_convertor(bbox_weight)
        bbox_weight = bbox_weight[None,:,:]
        surface_weights_candidates = self.bbox_parameter_encoder(bbox_weight, bbox_weight, init_index_weight)
        surface_weights_decoding = self.bbox_parameter_decoder(surface_weights_candidates, surface_weights_candidates,
                                                               init_index_weight)
        body_surface_weight = self.bbox_to_param_predictor(surface_weights_decoding.squeeze())
        body_surface_weight = body_surface_weight.reshape((self.bbox_surface_transfer_matrix.size(0), self.dim_in, 3, 3))
        return body_surface_weight
    ...
    def generate_surface_weights_from_part(self, part_weight):
        ...
        surface_weights_candidates = self.part_parameter_encoder(part_surface_weight, part_surface_weight, init_index_weight)
        surface_weights_decoding = self.part_parameter_decoder(surface_weights_candidates, surface_weights_candidates,
                                                               init_index_weight)
        body_surface_weight = self.part_to_param_predictor(surface_weights_decoding.squeeze())
    ...
    def forward(self, head_outputs, inter_outputs, task_params=None):
        ...
        for i in range(2):
            ...
            inter_ann_index = getattr(self, layer_name)(inter_features[i])
            inter_out.append(inter_ann_index)
        index_uv = inter_out2[1]#(index_uv+inter_out1[1]+inter_out2[1])*0.33
        return (ann_index, index_uv, u, v, m), inter_out1, inter_out2
